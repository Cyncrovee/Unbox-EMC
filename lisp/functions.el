...
(defun unbox-emc-start ()
  ...
  (if (get-buffer "*start*")
	...
	(with-current-buffer (get-buffer-create "*start*")
	  ...
	  (newline)
	  (insert "Most of names for variables in the user.el file have been changed. If you're getting related errors, simply swap out 'user' for 'unbox' in all user.el variables except user-boot-theme\n")
	  (read-only-mode)
	  (goto-char (point-min)))
	(switch-to-buffer "*start*")
